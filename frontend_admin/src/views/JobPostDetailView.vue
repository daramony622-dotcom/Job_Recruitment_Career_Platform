...
<template>
  <div class="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
    ...
    <div v-if="loading" class="text-center py-20 text-slate-400">Loading job post...</div>
    ...
    <template v-else-if="job">
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-2xl px-5 sm:px-8 py-7 sm:py-8">
        <div class="flex flex-col sm:flex-row sm:items-center gap-5">
          ...
          >
            <Briefcase class="w-10 h-10 text-blue-400" />
          </div>
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2.5">
              <h2 class="text-2xl sm:text-3xl font-bold text-slate-50 truncate">{{ job.title }}</h2>
              <StatusBadge :value="job.status" />
              <span
                v-if="job.is_featured"
                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-amber-500/15 text-amber-400 border border-amber-500/30"
              >
                <BadgeCheck class="w-3.5 h-3.5" /> Featured
              </span>
            </div>
            <p class="text-blue-400 font-medium mt-1.5">{{ job.company?.name || '—' }}</p>
        ...
        <div class="mt-7 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
          <div class="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-800/50 border border-slate-800">
            ...
            <span class="text-slate-300 truncate">{{ job.category?.name || '—' }}</span>
          </div>
          <div class="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-800/50 border border-slate-800">
            <Users class="w-4 h-4 text-slate-500 shrink-0" />
            <span class="text-slate-300 truncate">{{ job.vacancies }} openings</span>
          </div>
          <div class="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-800/50 border border-slate-800">
      ...
      <div class="mt-6 bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8">
        ...
        <p class="text-slate-300 leading-relaxed text-[15px] whitespace-pre-line">
          {{ job.description || 'No description available.' }}
        </p>
      </div>

      <!-- Requirements -->
      <div v-if="job.requirements" class="mt-6 bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8">
        <h3 class="text-lg font-bold text-slate-50 flex items-center gap-2.5 mb-4">
          <BadgeCheck class="w-5 h-5 text-blue-400" />
          Requirements
        </h3>
        <p class="text-slate-300 leading-relaxed text-[15px] whitespace-pre-line">{{ job.requirements }}</p>
      </div>

      <!-- Benefits -->
      <div v-if="job.benefits" class="mt-6 bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8">
        <h3 class="text-lg font-bold text-slate-50 flex items-center gap-2.5 mb-4">
          <Banknote class="w-5 h-5 text-blue-400" />
          Benefits
        </h3>
        <p class="text-slate-300 leading-relaxed text-[15px] whitespace-pre-line">{{ job.benefits }}</p>
      </div>

      <!-- Skills -->
      <div v-if="job.skills && job.skills.length" class="mt-6 bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8">
        <h3 class="text-lg font-bold text-slate-50 flex items-center gap-2.5 mb-4">
