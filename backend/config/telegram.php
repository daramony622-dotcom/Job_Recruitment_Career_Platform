...
/*
|--------------------------------------------------------------------------
| NOTE: irazasyed/telegram-bot-sdk config
|--------------------------------------------------------------------------
| This package is separate from the app's own Telegram login flow
| (App\Services\Telegram, Auth\TelegramAuthController,
| Auth\TelegramWebhookController, routes at /auth/telegram/* and
| /telegram/webhook). Both point at the same bot via TELEGRAM_BOT_TOKEN,
| but they are independent integrations. Do NOT also register this
| package's own webhook handler (Telegram::commandsHandler()) on the
| same webhook URL — that would double-process every update alongside
| TelegramWebhookController.
|
| If this package isn't actually used anywhere (no ->commandsHandler()
| call, no artisan telegram:* commands, no Telegram:: facade usage),
| consider removing it and this file entirely to avoid the confusion.
*/
