...
const isCompanyScopedUser = computed(() => {
  try {
    const user = JSON.parse(localStorage.getItem('admin_user') || localStorage.getItem('user') || 'null')
  ...
  try {
    if (isCompanyScopedUser.value) {
      try {
        ...
        if (profileError.response?.status === 404) return
        throw profileError
      }
    }
