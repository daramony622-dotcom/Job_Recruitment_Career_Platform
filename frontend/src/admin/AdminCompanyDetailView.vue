        ...
        <div class="flex flex-col gap-5 sm:block">
          ...
          <div class="min-w-0 sm:pr-60">
            <div class="flex flex-wrap items-center gap-2.5">
              <h2 class="notranslate truncate text-2xl font-black tracking-tight text-slate-950 dark:text-white sm:text-3xl" translate="no">
                {{ company.name }}
              </h2>
              <BadgeCheck v-if="company.is_verified" class="h-5 w-5 text-blue-600" title="Verified Employer" />
            </div>
            <p class="notranslate mt-1.5 font-semibold text-blue-600 dark:text-blue-400" translate="no">{{ company.industry || 'Company profile' }}</p>
            <p class="mt-1 inline-flex items-center gap-1.5 text-sm text-slate-600 dark:text-slate-400">
              <MapPin class="w-3.5 h-3.5" />
              <span class="notranslate" translate="no">{{ company.city || '—' }}{{ company.city && company.country ? ', ' : '' }}{{ company.country || '' }}</span>
            </p>
          </div>
