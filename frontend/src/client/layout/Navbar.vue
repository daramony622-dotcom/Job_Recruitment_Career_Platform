...
const { listNotifications } = useJobSeekerApi()
...
async function refreshNotificationCount() {
  ...
  try {
    ...
    unreadNotifications.value = Number(response?.unread_count || 0)
  } catch {
    // Keep the current badge state when notification polling is unavailable.
  }
}
...
const canAccessDashboard = computed(() => {
  return ['admin', 'hr', 'company'].includes(user?.value?.role)
})
...
onMounted(() => {
  ...
  notificationPoller = window.setInterval(refreshNotificationCount, 30000)
})
        ...
        <div class="flex items-center gap-1.5 sm:gap-2">
          ...
          >
            ...
            <span
              ...
              class="absolute top-2 right-2 w-2 h-2 bg-rose-500 rounded-full ring-2 ring-white dark:ring-slate-950"
          ...
          <template v-if="!isAuthenticated">
            ...
            <router-link
              to="/register"
              class="hidden sm:flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-600/20 transition-all duration-200 active:scale-95 cursor-pointer"
            >
          ...
          <template v-else>
            <a
              ...
              class="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-amber-500/10 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400 border border-amber-500/30 hover:bg-amber-500 hover:text-white transition-all duration-200"
            ...
            <router-link
              to="/profile"
              class="hidden sm:flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-300 hover:bg-slate-100 dark:hover:bg-slate-800/70 transition-all duration-200"
            >
              <div class="w-6.5 h-6.5 rounded-full overflow-hidden bg-blue-600 text-white text-[10px] font-black flex items-center justify-center shrink-0 shadow-xs">
        ...
        <div class="border-t border-slate-200/60 dark:border-slate-800/50 pt-3 mt-3 flex flex-col gap-2">
          ...
          <template v-else>
            <a
              ...
              :href="adminDashboardUrl"
              @click="mobileOpen = false"
              class="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 text-sm font-bold transition-all"
            >
              <ShieldCheck class="w-4 h-4" /> Dashboard
            </a>
