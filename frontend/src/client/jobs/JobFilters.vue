<script setup>
import { ref, onMounted } from 'vue'
import {
  SlidersHorizontal, ChevronDown, RotateCcw, Check, X,
  MapPin, DollarSign, Clock, FolderTree, Tag
} from 'lucide-vue-next'
import { useAuth } from '../../composables/useAuth'

const category = defineModel('category', { type: String, default: '' })
const skill = defineModel('skill', { type: String, default: '' })
const location = defineModel('location', { type: String, default: '' })
const salary = defineModel('salary', { type: String, default: '' })
const time = defineModel('time', { type: String, default: '' })

defineProps({
  totalJobs: { type: Number, default: 0 },
  hasActiveFilters: { type: Boolean, default: false }
})

const { request } = useAuth()
const categoriesList = ref([])
const skillsList = ref([])

const locations = [
  "Khan Boeung Keng Kang",
  "Khan Chamkar Mon",
  "Khan Chbar Ampov",
  "Khan Chroy Changvar",
  "Khan Dangkao",
  "Khan Daun Penh",
  "Khan Kamboul",
  "Khan Mean Chey",
  "Khan Pou Senchey",
  "Khan Prampi Makara",
  "Khan Prek Pnov",
  "Khan Russey Keo",
  "Khan Sen Sok",
  "Khan Tuol Kouk",
]

const salaryRanges = [
  '200$-500$',
  '500$-1000$',
  '1000$-1500$',
  '1500$-2000$',
  '2000$-3000$'
]

const timeRanges = [
  '1 day ago',
  '2 days ago',
  '3 days ago',
  '1 week ago',
  '2 weeks ago',
  '1 month ago'
]

const emit = defineEmits(['reset'])

const openDropdown = ref(null)

const toggleDropdown = (name) => {
  openDropdown.value = openDropdown.value === name ? null : name
}

async function fetchCategories() {
  try {
    const [categoryResponse, skillResponse] = await Promise.all([
      request('/categories'),
      request('/skills'),
    ])
    const categoryPayload = categoryResponse?.data ?? categoryResponse
    const skillPayload = skillResponse?.data ?? skillResponse
    const list = categoryPayload?.all || categoryPayload?.categories || categoryPayload?.job_categories || []
    categoriesList.value = Array.isArray(list) ? list : []
    skillsList.value = Array.isArray(skillPayload) ? skillPayload : (skillPayload?.data || [])
  } catch {
    // Non-blocking
  }
}

onMounted(fetchCategories)
</script>

<template>
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#0d1526] p-3 rounded-2xl border border-slate-200/80 dark:border-slate-800/70 shadow-xs relative">
    <div class="flex flex-wrap items-center gap-2">

      <div class="flex items-center gap-2 px-3.5 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-200 text-sm font-semibold select-none">
        <SlidersHorizontal class="w-4 h-4 text-slate-600 dark:text-slate-300" />
        <span>Filters</span>
      </div>

      <!-- Category Filter Dropdown -->
      <div class="relative">
        <button 
          @click="toggleDropdown('category')"
          type="button"
          class="flex items-center justify-between gap-2 rounded-xl px-3.5 py-2 text-sm font-medium transition cursor-pointer"
          :class="category 
            ? 'bg-slate-100 dark:bg-slate-800 text-blue-600 dark:text-blue-400 border border-transparent'
            : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 text-slate-700 dark:text-slate-200'"
        >
          <div class="flex items-center gap-2 truncate max-w-[140px] sm:max-w-[180px]">
            <FolderTree class="w-4 h-4 text-blue-500 shrink-0" />
            <span class="truncate">{{ category || 'Category & Skills' }}</span>
          </div>
          <div class="flex items-center gap-1 shrink-0">
            <span 
              v-if="category" 
              @click.stop="category = ''"
              class="p-0.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md text-slate-800 dark:text-slate-200 transition"
              title="Clear category"
            >
              <X class="w-4 h-4" />
            </span>
            <ChevronDown v-else class="w-4 h-4 text-slate-400" />
          </div>
        </button>

        <div v-if="openDropdown === 'category'" class="absolute left-0 mt-2 w-64 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-50 overflow-hidden">
          <div class="max-h-64 overflow-y-auto p-1 divide-y divide-slate-100 dark:divide-slate-800/60">
            <div class="p-1">
              <button
                @click="category = ''; openDropdown = null"
                class="w-full text-left px-3 py-2 text-xs font-bold rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200"
              >
                All Categories
              </button>
            </div>
            <div class="p-1 space-y-0.5">
              <button
                v-for="cat in categoriesList"
                :key="cat.id || cat.slug || cat.name"
                @click="category = cat.slug || cat.name; openDropdown = null"
                class="w-full text-left px-3 py-2 text-xs font-medium rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-between transition-colors"
                :class="(category === cat.slug || category === cat.name) ? 'bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 font-bold' : 'text-slate-700 dark:text-slate-200'"
              >
                <div class="min-w-0 pr-2">
                  <p class="notranslate truncate" translate="no">{{ cat.name || cat.title }}</p>
                  <p v-if="cat.roles || cat.jobs_count" class="text-[10px] text-slate-400">{{ cat.roles || `${cat.jobs_count} roles` }}</p>
                </div>
                <Check v-if="category === cat.slug || category === cat.name" class="w-4 h-4 text-blue-600 shrink-0" />
              </button>
            </div>
          </div>
        </div>
      </div>

      <!-- Skill Filter Dropdown -->
      <div class="relative">
        <button
          @click="toggleDropdown('skill')"
          type="button"
          class="flex items-center justify-between gap-2 rounded-xl px-3.5 py-2 text-sm font-medium transition cursor-pointer"
          :class="skill
            ? 'bg-slate-100 dark:bg-slate-800 text-purple-600 dark:text-purple-400 border border-transparent'
            : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 text-slate-700 dark:text-slate-200'"
        >
          <div class="flex items-center gap-2 truncate max-w-[140px] sm:max-w-[180px]">
            <Tag class="w-4 h-4 text-purple-500 shrink-0" />
            <span class="truncate">{{ skill || 'Skills' }}</span>
          </div>
          <div class="flex items-center gap-1 shrink-0">
            <span v-if="skill" @click.stop="skill = ''" class="p-0.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md" title="Clear skill">
              <X class="w-4 h-4" />
            </span>
            <ChevronDown v-else class="w-4 h-4 text-slate-400" />
          </div>
        </button>

        <div v-if="openDropdown === 'skill'" class="absolute left-0 mt-2 w-64 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-50 overflow-hidden">
          <div class="max-h-64 overflow-y-auto p-1">
            <button @click="skill = ''; openDropdown = null" class="w-full text-left px-3 py-2 text-xs font-bold rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200">
              All Skills
            </button>
            <button
              v-for="item in skillsList"
              :key="item.id || item.slug || item.name"
              @click="skill = item.slug || item.name; openDropdown = null"
              class="w-full text-left px-3 py-2 text-xs font-medium rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-between"
              :class="skill === (item.slug || item.name) ? 'bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-400 font-bold' : 'text-slate-700 dark:text-slate-200'"
            >
              <span class="notranslate truncate" translate="no">{{ item.name }}</span>
              <Check v-if="skill === (item.slug || item.name)" class="w-4 h-4 text-purple-600 shrink-0" />
            </button>
            <p v-if="skillsList.length === 0" class="px-3 py-3 text-xs text-slate-500">No skills available.</p>
          </div>
        </div>
      </div>

      <!-- Location Dropdown -->
      <div class="relative">
        <button 
          @click="toggleDropdown('location')"
          type="button"
          class="flex items-center justify-between gap-2 rounded-xl px-3.5 py-2 text-sm font-medium transition cursor-pointer"
          :class="location 
            ? 'bg-slate-100 dark:bg-slate-800 text-blue-600 dark:text-blue-400 border border-transparent'
            : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 text-slate-700 dark:text-slate-200'"
        >
          <div class="flex items-center gap-2 truncate">
            <MapPin v-if="location" class="w-4 h-4 text-blue-500 shrink-0" />
            <span class="truncate">{{ location || 'Location' }}</span>
          </div>
          <div class="flex items-center gap-1 shrink-0">
            <span 
              v-if="location" 
              @click.stop="location = ''"
              class="p-0.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md text-slate-800 dark:text-slate-200 transition"
              title="Clear location"
            >
              <X class="w-4 h-4" />
            </span>
            <ChevronDown v-else class="w-4 h-4 text-slate-400" />
          </div>
        </button>

        <div v-if="openDropdown === 'location'" class="absolute left-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg z-50 overflow-hidden">
          <div class="max-h-52 overflow-y-auto p-1">
            <button
              @click="location = ''; openDropdown = null"
              class="w-full text-left px-3 py-2 text-sm font-medium rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200"
            >
              Location (All)
            </button>
            <button
              v-for="loc in locations"
              :key="loc"
              @click="location = loc; openDropdown = null"
              class="w-full text-left px-3 py-2 text-sm font-medium rounded-lg hover:bg-slate-100 flex items-center justify-between"
              :class="location === loc ? 'bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 font-semibold' : 'text-slate-700 dark:text-slate-200'"
            >
              <span class="truncate">{{ loc }}</span>
              <Check v-if="location === loc" class="w-4 h-4 text-blue-600 shrink-0" />
            </button>
          </div>
        </div>
      </div>

      <!-- Salary Dropdown -->
      <div class="relative">
        <button 
          @click="toggleDropdown('salary')"
          type="button"
          class="flex items-center justify-between gap-2 rounded-xl px-3.5 py-2 text-sm font-medium transition cursor-pointer"
          :class="salary 
            ? 'bg-slate-100 dark:bg-slate-800 text-blue-600 dark:text-blue-400 border border-transparent'
            : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 text-slate-700 dark:text-slate-200'"
        >
          <div class="flex items-center gap-2 truncate">
            <DollarSign v-if="salary" class="w-4 h-4 text-blue-500 shrink-0" />
            <span class="truncate">{{ salary || 'Any salary' }}</span>
          </div>
          <div class="flex items-center gap-1 shrink-0">
            <span 
              v-if="salary" 
              @click.stop="salary = ''"
              class="p-0.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md text-slate-800 dark:text-slate-200 transition"
              title="Clear salary"
            >
              <X class="w-4 h-4" />
            </span>
            <ChevronDown v-else class="w-4 h-4 text-slate-400" />
          </div>
        </button>

        <div v-if="openDropdown === 'salary'" class="absolute left-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg z-50 overflow-hidden">
          <div class="max-h-52 overflow-y-auto p-1">
            <button
              @click="salary = ''; openDropdown = null"
              class="w-full text-left px-3 py-2 text-sm font-medium rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200"
            >
              Any salary
            </button>
            <button
              v-for="sal in salaryRanges"
              :key="sal"
              @click="salary = sal; openDropdown = null"
              class="w-full text-left px-3 py-2 text-sm font-medium rounded-lg hover:bg-slate-100 flex items-center justify-between"
              :class="salary === sal ? 'bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 font-semibold' : 'text-slate-700 dark:text-slate-200'"
            >
              <span>{{ sal }}</span>
              <Check v-if="salary === sal" class="w-4 h-4 text-blue-600 shrink-0" />
            </button>
          </div>
        </div>
      </div>
      
      <!-- Time Dropdown -->
      <div class="relative">
        <button 
          @click="toggleDropdown('time')"
          type="button"
          class="flex items-center justify-between gap-2 rounded-xl px-3.5 py-2 text-sm font-medium transition cursor-pointer"
          :class="time 
            ? 'bg-slate-100 dark:bg-slate-800 text-blue-600 dark:text-blue-400 border border-transparent'
            : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 text-slate-700 dark:text-slate-200'"
        >
          <div class="flex items-center gap-2 truncate">
            <Clock v-if="time" class="w-4 h-4 text-blue-500 shrink-0" />
            <span class="truncate">{{ time || 'Any time' }}</span>
          </div>
          <div class="flex items-center gap-1 shrink-0">
            <span 
              v-if="time" 
              @click.stop="time = ''"
              class="p-0.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md text-slate-800 dark:text-slate-200 transition"
              title="Clear time"
            >
              <X class="w-4 h-4" />
            </span>
            <ChevronDown v-else class="w-4 h-4 text-slate-400" />
          </div>
        </button>

        <div v-if="openDropdown === 'time'" class="absolute left-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg z-50 overflow-hidden">
          <div class="max-h-52 overflow-y-auto p-1">
            <button
              @click="time = ''; openDropdown = null"
              class="w-full text-left px-3 py-2 text-sm font-medium rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200"
            >
              Any time
            </button>
            <button
              v-for="t in timeRanges"
              :key="t"
              @click="time = t; openDropdown = null"
              class="w-full text-left px-3 py-2 text-sm font-medium rounded-lg hover:bg-slate-100 flex items-center justify-between"
              :class="time === t ? 'bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 font-semibold' : 'text-slate-700 dark:text-slate-200'"
            >
              <span>{{ t }}</span>
              <Check v-if="time === t" class="w-4 h-4 text-blue-600 shrink-0" />
            </button>
          </div>
        </div>
      </div>

      <!-- Clear All Button -->
      <button 
        v-if="hasActiveFilters"
        @click="emit('reset')"
        type="button"
        class="flex items-center gap-1.5 px-3 py-2 text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-xl text-xs font-semibold transition active:scale-95"
      >
        <RotateCcw class="w-3.5 h-3.5" />
        <span>Clear Filters</span>
      </button>

    </div>

    <div class="text-xs font-semibold text-slate-500 self-end md:self-center pr-1">
      {{ totalJobs }} jobs found
    </div>
  </div>
</template>