...
<template>
  <div class="space-y-4">
    <div 
      ...
      :key="job.id"
      class="bg-white dark:bg-[#0d1526] rounded-3xl border border-slate-200/80 dark:border-slate-800/70 p-6 md:p-7 shadow-xs hover:shadow-lg hover:border-blue-300 dark:hover:border-blue-700/60 transition-all duration-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden group"
    >
      <!-- Featured Top Badge -->
      <div v-if="job.is_featured" class="absolute top-0 right-0 bg-blue-700 text-white text-[10px] font-bold uppercase px-3.5 py-1 rounded-bl-2xl tracking-wider flex items-center gap-1 shadow-xs border-b border-l border-blue-600/40">
      ...
      <div class="flex items-start gap-4 flex-1 min-w-0">
        ...
        <div class="shrink-0 mt-0.5">
          ...
          <div 
            v-else 
            class="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-700 to-indigo-800 text-white font-extrabold text-base flex items-center justify-center shadow-md shadow-blue-900/20 group-hover:scale-105 transition transform duration-200 border border-blue-600/40"
          >
        ...
        <div class="space-y-2.5 min-w-0 flex-1">
          ...
          <div class="flex items-center gap-2 flex-wrap">
            <span v-if="job.category_name" class="px-2.5 py-0.5 rounded-lg text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
              <span class="notranslate" translate="no">{{ job.category_name }}</span>
            </span>
          ...
          <router-link :to="`/jobs/${job.id}`">
            <h2 class="text-base sm:text-lg font-extrabold text-slate-900 dark:text-white tracking-tight truncate hover:text-blue-700 dark:hover:text-blue-300 transition cursor-pointer">
              <span class="notranslate" translate="no">{{ job.title }}</span>
            </h2>
          </router-link>
          ...
          <div class="flex items-center gap-4 flex-wrap text-xs text-slate-600 dark:text-slate-400 font-medium">
            <span v-if="job.company_name" class="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1">
              <Building2 class="w-3.5 h-3.5 text-blue-700 dark:text-blue-400" />
              <span class="notranslate" translate="no">{{ job.company_name }}</span>
            </span>
          ...
          <div class="flex items-center gap-4 flex-wrap text-xs text-slate-500 dark:text-slate-400 pt-1 border-t border-slate-100 dark:border-slate-800/80">
            <span class="flex items-center gap-1 font-extrabold text-blue-700 dark:text-blue-300">
      ...
      <div class="flex items-center gap-2 w-full md:w-auto justify-end pt-3 md:pt-0 border-t md:border-t-0 border-slate-100 dark:border-slate-800 shrink-0">
        <button
          ...
          :class="savedJobs.has(Number(job.id)) ? 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/40 dark:text-blue-300' : ''"
          title="Save Job"
        >
        ...
        <button 
          ...
          type="button"
          class="flex-1 md:flex-initial flex items-center justify-center gap-1.5 px-5 py-3 bg-blue-700 hover:bg-blue-800 text-white rounded-2xl text-xs font-bold transition shadow-md shadow-blue-700/20 active:scale-95 cursor-pointer border border-blue-600/40"
        >
