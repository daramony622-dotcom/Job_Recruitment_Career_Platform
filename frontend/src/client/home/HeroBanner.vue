...
<template>
  <section class="site-banner site-banner-hero relative bg-cover bg-center flex items-center px-4 sm:px-8 py-12 sm:py-16 text-white overflow-hidden rounded-3xl bg-[url('https://images.unsplash.com/photo-1521737711867-e3b97375f902?q=80&w=1920&auto=format&fit=crop')] shadow-xl my-2">
    
    <!-- Neutral Overlay: Shows full clear image without blue tint -->
    ...
    <div class="relative z-10 max-w-5xl mx-auto w-full space-y-8">
      ...
      <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/85 backdrop-blur-md border border-white/70 text-xs font-bold text-slate-800 shadow-sm">
        ...
        <span>Over 12,400 active job opportunities posted this month</span>
      </div>
      ...
      <div class="space-y-4 max-w-3xl">
        <h1 class="site-banner-title font-extrabold tracking-tight text-white">
          ...
          <span class="text-transparent bg-clip-text bg-gradient-to-r from-sky-300 via-cyan-200 to-indigo-200">dream career role</span>
        </h1>

        <p class="site-banner-copy text-slate-100 font-medium">
      ...
      <form @submit.prevent="handleSearch" class="bg-white/95 dark:bg-slate-900/95 p-2 rounded-2xl shadow-2xl backdrop-blur-xl border border-white/20 dark:border-slate-800 flex flex-col md:flex-row items-center gap-2 max-w-3xl">
        ...
        <div class="flex items-center gap-3 px-3.5 py-2.5 w-full md:w-1/2 bg-slate-50 dark:bg-slate-800/80 md:bg-transparent rounded-xl">
          ...
          <input 
            ...
            class="w-full bg-transparent text-slate-800 dark:text-slate-100 placeholder-slate-400 text-xs sm:text-sm focus:outline-none font-medium"
          />
        </div>
        ...
        <div class="flex items-center gap-2 w-full md:w-auto shrink-0">
          <button 
            v-if="jobTitle || location"
            @click="clearInputs"
            type="button"
            class="p-3 text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition flex items-center justify-center cursor-pointer"
            title="Clear inputs"
          >
            <X class="w-5 h-5" />
          </button>
          ...
          <button 
            type="submit"
            class="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-3 rounded-xl transition flex items-center justify-center gap-2 text-xs sm:text-sm shadow-md shadow-blue-600/20 active:scale-95 cursor-pointer shrink-0"
          >
      ...
      <!-- Live Stats Grid -->
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-6 border-t border-white/15 max-w-3xl">
        <div class="flex items-center gap-3">
          <div class="p-2.5 bg-white/10 rounded-xl text-blue-400 shrink-0">
